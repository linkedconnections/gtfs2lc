      ...
      for (let i = 0; i < numCPUs; i++) {
        const worker = new Worker(__filename, {
          workerData: {
            ...
            path: path,
            output: output,
            routesdb: options.store === 'MemStore' ? routesdb : path + '/.routes',
            servicesdb: options.store === 'MemStore' ? servicesdb : path + '/.services',
            stopsdb: options.store === 'MemStore' ? stopsdb : path + '/.stops',
            options: options
          }
        });
        ...
        worker.on('message', () => {
          ...
          if (w === numCPUs) {
            ...
            exec('cat raw_* > linkedConnections.' + ext, { cwd: output }, err => {
              if (err) {
                throw err;
              }
              let t1 = new Date();
