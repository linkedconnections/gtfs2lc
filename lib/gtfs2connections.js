...
Mapper.prototype.resultStream = function (path, output, done) {
  ...
  tripsIterator.transform((trip, doneTrip) => {
    if (trip['trip_id']) {
      if (tripsdb instanceof Map) {
        tripsdb.set(trip['trip_id'], trip);
        doneTrip();
      } else {
        tripsdb.set(trip['trip_id'], trip).then(function () {
          doneTrip();
        });
      }
    } else {
...
if (!isMainThread) {
  ...
  let connectionsBuilder = new ConnectionsBuilder(tripsdb, servicesdb, routesdb, workerData['options']);
