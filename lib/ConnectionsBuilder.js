    ...
    try {
      ...
      for (var i in service) {
        ...
        const serviceDay = new Date(
          service[i].substr(0, 4),
          parseInt(service[i].substr(4, 2)) - 1,
          service[i].substr(6, 2),
        );
        ...
        const connection = {
          ...
          stop_sequence: connRule.stop_sequence,
...
const parseGTFSDuration = function (durationString) {
  let [hours, minutes, seconds] = durationString.split(":").map((val) => {
    return parseInt(val);
  });
...
const addDuration = function (date, duration) {
  return addSeconds(
    addMinutes(addHours(date, duration.hours), duration.minutes),
    duration.seconds,
  );
};
