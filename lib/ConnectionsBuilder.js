...
ConnectionsBuilder.prototype._transform = function (connectionRule, encoding, done) {
  ...
  var tripIds = connectionRule.trip_id.split('|&AND&|');
  if (tripIds.length > 1) {
    var callbackCount = 0;
    for (var tripId of tripIds) {
      var connectionRuleCopy = JSON.parse(JSON.stringify(connectionRule));
      connectionRuleCopy.trip_id = tripId;
      this.processConnectionRule(connectionRuleCopy, () => {
        callbackCount ++;
        if (callbackCount === tripIds.length) {
          done();
        }
      });
    }
