	...
	*/
	...
	async differentialUpdate(id, serviceDates) {
		try {
			...
			const newServices = serviceDates.filter(s => !old.includes(s));
			if (newServices.length > 0) {
				await this.historyDB.put(id, [...old, ...newServices]);
				return newServices;
			} else {
				// Nothing to update
				return null;
			...
			if (err.code === 'LEVEL_NOT_FOUND') {
				// Is a completely new Connection rule, return all service dates
				await this.historyDB.put(id, serviceDates);
				return serviceDates;
			} else {
