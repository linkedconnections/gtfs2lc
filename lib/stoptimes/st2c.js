...
var StopTimesToConnections = function (stopsdb) {
  Transform.call(this, {objectMode : true});
  this._stopsdb = stopsdb;
  this._previousStopTime = null;
  this._firstDeparture = null;
};
 ...
 */
StopTimesToConnections.prototype._transform = async function (stopTime, encoding, done) {
  ...
  if (stopTime['drop_off_type'] !== null && stopTime['drop_off_type'] == 1  && stopTime['pickup_type'] !== null && stopTime['pickup_type'] == 1) {
    ...
    if (!this._previousStopTime) {
      ...
      this._firstDeparture = stopTime['departure_time'];
    } else {
      if (this._previousStopTime['trip_id'] === stopTime['trip_id']) {
        ...
        this.push({
          ...
          first_departure_dfm: this._firstDeparture,
          arrival_dfm: stopTime['arrival_time'],
          departure_dfm: this._previousStopTime['departure_time'],
          departure_stop: await this._stopsdb.get(this._previousStopTime['stop_id']),
          arrival_stop : await this._stopsdb.get(stopTime['stop_id']),
