...
class StopTimesToConnections extends Transform {
   ...
   */
  _transform(stopTime, encoding, done) {
    this.processStopTime(stopTime).then(() => done(), done);
  }

  async processStopTime(stopTime) {
    if (
      this.previousStopTime &&
      this.previousStopTime["trip_id"] === stopTime["trip_id"]
    ) {
      if (
        ...
        console.error(
          `WARNING: Empty arrival and departure times found for trip ${stopTime["trip_id"]} on stop ${stopTime["stop_id"]}. Interpolation is required in a previous step to handle these cases (not supported yet). This stop time will be skipped.`,
        );
      ...
      const { departureStop, arrivalStop, trip, route, serviceDates } =
        await this.getRelatedData(
          this.previousStopTime["stop_id"],
          stopTime["stop_id"],
        );
      ...
      const depStopId =
        departureStop["stop_code"] && departureStop["stop_code"] !== ""
          ? departureStop["stop_code"]
          : departureStop["stop_id"];
      const arrStopId =
        arrivalStop["stop_code"] && arrivalStop["stop_code"] !== ""
          ? arrivalStop["stop_code"]
          : arrivalStop["stop_id"];
  ...
  async differentialUpdate(id, serviceDates) {
    try {
      ...
      const newServices = serviceDates.filter(
        (s) => !oldServiceDates.includes(s),
      );
