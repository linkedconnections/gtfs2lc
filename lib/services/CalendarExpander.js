...
class CalendarExpander extends Transform {
  ...
  _transform(calendar, encoding, done) {
    ...
    if (calDates) {
      ...
      calDates.added.forEach((d) =>
        expanded.add(format(this.createDate(d), "yyyyMMdd")),
      );

      for (const d of days) {
        // Check this date is an actual service date and it hasn't been removed
        if (
          calendar[format(d, "iiii").toLowerCase()] === "1" &&
          !calDates.removed.has(format(d, "yyyyMMdd"))
        ) {
  ...
  _flush(done) {
    ...
    for (const [service_id, obj] of this.calendarDates) {
      const dates = [];
      obj.added.forEach((d) =>
        dates.push(format(this.createDate(d), "yyyyMMdd")),
      );
