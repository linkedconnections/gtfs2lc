...
describe("URIStrategy", () => {
  describe("getRouteId", () => {
    ...
    it("rejects executable resolver expressions", () => {
      const strategy = new URIStrategy({
        route: "http://example.org/routes/{route_id}",
        resolve: { route_id: "process.exit(1)" },
      });
      expect(() =>
        strategy.getRouteId({ route: { route_id: "route" } }),
      ).toThrow("Unsupported base URI resolver expression");
    });
    ...
    it("Should resolve stop URI", async () => {
      ...
      assert.equal(
        await strategy.getStopId({ stop_id: "stop1" }),
        "http://example.org/stops/stop1",
      );
    });

    it("supports legacy entity aliases in resolver expressions", () => {
      const strategy = new URIStrategy({
        trip: "http://example.org/trips/{start}",
        resolve: { start: "format(trips.startTime, 'yyyyMMdd');" },
      });
      expect(
        strategy.getTripId({ trip: { startTime: new Date("2020-02-15") } }),
      ).toBe("http://example.org/trips/20200215");
    });
    ...
    it("Should resolve trip URI", () => {
      ...
      assert.equal(
        strategy.getTripId(connection),
        "http://example.org/trips/trip1/20200215/2020",
      );
    });
  });
    ...
    it("should resolve expression using date-fns.format function", () => {
      const strategy = new URIStrategy({
        ...
        resolve: {
          ...
          trip_startTime:
            "format(connection.trip.startTime, \"yyyyMMdd'T'HHmm\");",
