...
describe("The file connections.nldjsonld should contain things", () => {
  const readConnections = async function () {
    const contents = await readFile(
      "test/sample-feed/connections.nldjsonld",
      "utf8",
    );
    return contents
      .split(/\r?\n/u)
  ...
  var streamPromise = readConnections();
  var connections = [];
  it("Joining trips should give no less or more connections than expected", async () => {
    ...
    let joiningtrip = connections.filter((connection) => {
      return (
        connection["gtfs:route"] ===
          "http://example.org/routes/joining_route" &&
        connection.departureTime.substr(0, 10) === "2007-02-17"
      );
    });
    ...
    assert.equal(
      joiningtrip.filter(
        (connection) =>
          connection.departureStop === "http://example.org/stops/D",
      ).length,
      1,
    );
  ...
  it("A non joining trip that was flagged potentially joining should show up separately", () => {
    ...
    let nonjoiningtrip = connections.filter((connection) => {
      return (
        connection["@id"] ===
        "http://example.org/connections/20070216/C/non_joining_splitting_trip_3"
      );
    });
  ...
  it("A splitting trip that was flagged potentially joining should show up as a splitting trip", () => {
    ...
    let splittingtrip = connections.filter((connection) => {
      return (
        connection["gtfs:trip"] ===
        "http://example.org/trips/non_joining_splitting_trip_3/20070216"
      );
    });
